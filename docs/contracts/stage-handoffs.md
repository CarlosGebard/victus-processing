...
- Inputs are structured paper metadata and blocks.
- Outputs contain only `metadata` and `blocks`.
...
- Output is an experiment map generated by `experiment_scope_mapper`.
...
- Inputs are trimmed `metadata` and `blocks`, plus the validated
  `experiment_map`.
