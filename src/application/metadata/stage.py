...
def run_metadata_exploration_flow(mode: str = "broad-nutrition", *, llm_client: LLMClient) -> None:
    normalized_mode = normalize_selection_mode(mode)
    if normalized_mode == "broad-nutrition":
        run_nutrition_rag_exploration(llm_client=llm_client)
        return
    if normalized_mode == "dataset-gaps":
        run_dataset_gaps_exploration(llm_client=llm_client)
        return
